...
class Heating extends Calculator
{
    ...
    public function performCalculations(): array
    {
        ...
        foreach(['current' => 'heat-source', 'new' => 'new-heat-source'] as $situation => $heatSourceShort) {
            ...
            if ( ! in_array('hr-boiler', $heatSources) && ! in_array(
                ...
                Log::debug(__METHOD__ . ' - correcting situation (because no hr-boiler / district-heating => assign gas rest to wtw and not to heating');
                $wtw[$situation]['gas'] = $amountGas - $cooking[$situation]['gas'];
            }
        }

        Log::debug("End result: " . json_encode( [
                'heating'   => $heating,
                'tap_water' => $wtw,
                'cooking'   => $cooking,
            ]));
    ...
    protected function calculateHeating(
        ...
        if ( ! in_array('hr-boiler', $heatSources)) {
            Log::debug(__METHOD__ . ' - No HR boiler, returning' . json_encode($result));
            return $result;
        }
        ...
        Log::debug(__METHOD__ . ' - heatingGasUsage = ' . $heatingGasUsage . ' = ' . $amountGas . ' - ' . data_get($cookingUsage, 'gas', 0));
    ...
    protected function calculateTapWater(
        ...
        if (empty($primaryWtwHeatSourceShort) || $primaryWtwHeatSourceShort == 'none') {
            Log::debug(__METHOD__ . ' - No primary wtw heat source, returning' . json_encode($result));
            return $result;
        }
        ...
        if ( ! $consumption instanceof KeyFigureConsumptionTapWater) {
            Log::debug(__METHOD__ . ' - No key figure for tap water consumption, returning' . json_encode($result));
            return $result;
        }
        ...
        if (in_array('sun-boiler', $heatSourcesWtw)) {
            ...
            Log::debug(__METHOD__ . ' - solar boiler yield: ' . json_encode($solarBoilerYield));
        }
        ...
        if (in_array(
            ...
            Log::debug(__METHOD__ . ' - characteristic id = ' . optional($characteristics)->id);
            $scop                 = 1;
