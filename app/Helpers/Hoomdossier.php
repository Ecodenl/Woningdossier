...
class Hoomdossier
{
    ...
    public static function getMostCredibleValue(Relation $relation, $column, $default = null)
    {
        ...
        // if the user is not comparing his own data / values against a other input source his data / values
        // then we want to show him the most credible value
        // however if the user is comparing, we need to show him his own data otherwise there would not be anything to compare
        //
        if (HoomdossierSession::isUserNotComparingInputSources()) {
            ...
            $results = $found->pluck($column, 'short');
            foreach ($results as $inputSourceShort => $value) {
                // only return the value if the short is resident
                if ($inputSourceShort == InputSource::RESIDENT_SHORT) {
                    // no matter what
                    return $value;
                }
