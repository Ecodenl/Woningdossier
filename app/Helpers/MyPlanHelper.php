...
class MyPlanHelper
{
    ...
    public static function saveUserInterests(Request $request, UserActionPlanAdvice $advice) : string
    {
        ...
        // so its almost working
        // now set the years working
        $allAdvices = $request->input('advice.*.'.$step);
        if ($step == "insulated-glazing") {
            foreach ($allAdvices as $advices) {
                if (is_array($advices)) {
                    if (array_key_exists('interested', $advices)) {
                        $interested = true;
                    }
                }
            }
        ...
        // we only want to update the interest level if the measure type = energy saving.
        if ($advice->measureApplication->measure_type == "energy_saving") {

//            $advice->user->interests()->where('interest_id', 1)->orWhere('interest_id', 2)->update([
//                'interest_id' => 4,
//            ]);
