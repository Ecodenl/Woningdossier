    ...
    public function import()
    {
        ...
        foreach ($csvRows as $csvKey => $csvRow) {
            ...
            if ($csvRow[3] != "") {
                ...
                if (array_key_exists(4, $csvRow) && Str::isValidUuid($csvRow[4])) {
                    $translationUuidHelpKey = $csvRow[4];
                } else {
                    $translationUuidHelpKey = Str::uuid();
                }
                if (array_key_exists(5, $csvRow) && Str::isValidUuid($csvRow[5])) {
                ...
                //dump("Tabblad: {$csvRow[0]} Veldnaam: {$csvRow[1]} Uitleg: {$csvRow[2]} Short: {$short} uuid-help-key: {$translationUuidHelpKey} uuid-help-key: {$translationUuidTitleKey}");
                $csvData[] = [
                    $csvRow[0],
                    $csvRow[1],
                    $csvRow[2],
        ...
        dump("Created counter: {$createCounter}");
        dump("Update counter: {$updateCounter}");
        ...
        //dump('Writing the uuid translatable file...', "fg=blue");
        ...
        //dump($createCounter." translations have been created and ".$updateCounter." have been updated.");
        //dump('Done!', "fg=green");
    }
