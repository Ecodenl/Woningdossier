...
class CurrentRoleMiddleware
{
    ...
    public function handle($request, Closure $next, $role)
    {
        ...
        if (!\Auth::account()->user()->hasRole($authorizedRole)) {
        ...
        if (\Auth::account()->user()->hasMultipleRoles() && !\Auth::account()->user()->hasRoleAndIsCurrentRole($roles)) {
            return redirect(route('cooperation.admin.index'));
        }

        // check if the user has the role and if it is his current role.
        if (!\Auth::account()->user()->hasRoleAndIsCurrentRole($roles)) {
