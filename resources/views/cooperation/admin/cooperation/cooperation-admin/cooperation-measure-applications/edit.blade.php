...
@section('content')
    <form class="w-full flex flex-wrap"
          action="{{ route('cooperation.admin.cooperation.cooperation-admin.cooperation-measure-applications.update', compact('cooperationMeasureApplication')) }}"
          method="POST">
        @csrf
        @method('PUT')
        ...
        @php
            $old = old('cooperation_measure_applications.extra.icon', $cooperationMeasureApplication->extra['icon'] ?? 'icon-account-circle');
        @endphp
