...
<div class="col-sm-4">
    <div class="form-group">
        <select class="form-control validation" name="validation[{{$question->id}}][name]" id="">
...
<div class="col-sm-4">
    <div class="form-group">
        @foreach(__('woningdossier.cooperation.admin.custom-fields.index.rules') as $rule => $translation)
            <select class="form-control validation-options" name="validation[{{$question->id}}][validation-options]" @if($rule != $mainValidationRule) style="display: none;" @endif id="{{$rule}}">
...
@if(is_array($question->validation))
    @foreach($question->validation[$mainValidationRule] as $optionalRule => $rules)
        @foreach($rules as $validationOptionName => $validationOptionValue)
            <div class="col-sm-2">
                <div class="form-group">
                    <input type="text" name="validation[{{$question->id}}][validation-options][{{$optionalRule}}][{{$validationOptionName}}]" class="form-control" value="{{$validationOptionValue}}">
                </div>
            </div>
        @endforeach
    @endforeach
