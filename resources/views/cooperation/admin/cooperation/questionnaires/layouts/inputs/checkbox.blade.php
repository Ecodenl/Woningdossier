...
@foreach(config('hoomdossier.supported_locales') as $locale)
    ...
    <div class="form-group">
        <div class="input-group">
            ...
            <input name="questions[{{$question->id}}][question][{{$locale}}]" placeholder="@lang('cooperation/admin/cooperation/questionnaires.shared.types.default-placeholder')"
                   type="text" value="{{old("questions.{$question->id}.question.{$locale}", $translation)}}" class="form-control">
        </div>
    </div>
@endforeach
...
{{-- quick maths--}}
<label for="">@lang('cooperation/admin/cooperation/questionnaires.shared.types.default-option-label') {{$questionOptionCount + 1}} @lang('cooperation/admin/cooperation/questionnaires.shared.types.add')</label>
<?php $uuid = \Ramsey\Uuid\Uuid::uuid4(); ?>
@foreach(config('hoomdossier.supported_locales') as $locale)
    <div class="form-group">
        <div class="input-group">
            <span class="input-group-addon">{{$locale}}</span>
            <input name="questions[{{$question->id}}][options][{{$uuid}}][{{$locale}}]" placeholder="@lang('cooperation/admin/cooperation/questionnaires.shared.types.default-placeholder')"
