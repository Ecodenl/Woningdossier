@extends('cooperation.admin.layouts.app', [
    'panelTitle' => __('woningdossier.cooperation.admin.super-admin.key-figures.index.header')
])

@section('content')
    <div class="w-full data-table">
        <h2 class="heading-2 w-full mt-5 mb-2">
            @lang('woningdossier.cooperation.admin.super-admin.key-figures.index.sections.general')
        </h2>
        <table class="table fancy-table table-auto table-divided">
            <thead>
                <tr>
            ...
            <tbody>
                @foreach($keyfigures as $step => $keyfigureData)
                    @foreach($keyfigureData as $kengetalConstant => $kengetal)
                        ...
                        <tr>
                            <td>
                                @if($translationKey == $kengetalTitle)
                                    {!! $kengetalConstant !!}
                                @else
                                    {!! $kengetalTitle !!}
                                @endif
                            ...
                            <td>{{$kengetal}}</td>
                            <td>
                                @if($unitTranslationKey != $kengetalUnit)
                                    {!! __('key-figures.' . $step . '.' . $kengetalConstant .'.unit') !!}
                                @endif
                            </td>
                        </tr>
                    @endforeach
        ...
        <h2 class="heading-2 w-full mt-5 mb-2">
            @lang('woningdossier.cooperation.admin.super-admin.key-figures.index.sections.measure_applications')
        </h2>
        <table class="table fancy-table table-auto table-divided">
            <thead>
                <tr>
