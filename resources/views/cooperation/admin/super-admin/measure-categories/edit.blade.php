...
@section('content')
    <form class="w-full flex flex-wrap"
          action="{{route('cooperation.admin.super-admin.measure-categories.update', compact('measureCategory'))}}"
          method="POST">
        @csrf
        @method('PUT')
                ...
                <select id="vbjehuis-measure" name="vbjehuis_measure" class="form-input hidden"
                    ...
                    @foreach($measures as $measure)
                        <option value="{{ $measure['Value'] }}"
                                @if(old('vbjehuis_measure', $currentMapping->target_data['Value'] ?? null) == $measure['Value']) selected @endif
                        >
                            {{ $measure['Label'] }}
                        </option>
                    @endforeach
