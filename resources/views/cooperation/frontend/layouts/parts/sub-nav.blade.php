<div class="flex items-center justify-between w-full bg-blue-100 border-b-2 border-blue border-opacity-20 h-16 px-5 xl:px-20 relative z-30">
    @foreach($steps as $step)
        @php
            ...
            $route = route('cooperation.frontend.tool.quick-scan.index', ['scan' => $scan, 'step' => $step, 'subStep' => $step->subSteps()->orderBy('order')->first()]);
            if ($stepWasCompleted) {
                $route = route('cooperation.frontend.tool.quick-scan.index', ['scan' => $scan, 'step' => $step, 'subStep' => $step->subSteps()->orderByDesc('order')->first()]);
            }
        @endphp
        <a href="{{ $route }}"
           class="no-underline">
        ...
        @if($hasQuestionnaires)
            ...
            @foreach($questionnaires as $questionnaire)
                <a href="{{ route('cooperation.frontend.tool.quick-scan.questionnaires.index', compact('scan', 'step', 'questionnaire')) }}"
                   class="no-underline">
                    <div class="flex items-center">
    ...
    <a href="{{ route('cooperation.frontend.tool.quick-scan.my-plan.index', compact('scan')) }}" class="no-underline">
