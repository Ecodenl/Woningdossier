    ...
    @if(in_array($service->short, ['hr-boiler', 'boiler']))
        ...
        @if($boilerCount == 1)
            <div class="row">
                ...
                <div class="col-md-4 ">
                    @if($boilerCount == 1)
                        <img class="img-responsive mt-15 pr-10 d-inline pull-left"
                             src="{{asset('images/service-icons/'.$iconName.'.png')}}">
                    @endif
                    @component('cooperation.tool.components.step-question', ['id' => 'services.'.$service->short.'.service_value_id', 'translation' => 'cooperation/tool/general-data/current-state.index.service.'.$service->short])
                        @component('cooperation.tool.components.input-group', ['inputType' => 'select', 'inputValues' => $service->values, 'userInputValues' => $building->buildingservices()->forMe()->where('service_id', $service->id)->get(), 'userInputColumn' => 'service_value_id'])
                            <select id="service_{{ $service->id }}" class="form-control"
                    ...
                    <div class="col-md-4">
                        @component('cooperation.tool.components.step-question', ['id' => 'building_features-building_heating_application_id', 'translation' => 'cooperation/tool/general-data/current-state.index.building-heating-applications'])
                            @component('cooperation.tool.components.input-group', ['inputType' => 'select', 'inputValues' => $buildingHeatingApplications, 'userInputValues' => $myBuildingFeatures, 'userInputColumn' => 'building_heating_application_id'])
                                <select id="building_features-building_heating_application_id" class="form-control"
                                        name="building_features[building_heating_application_id]">
                                    {{--5 is the "vraaggestuurd" value, we need this for a checkbox--}}
                                    @foreach($buildingHeatingApplications as $buildingHeatingApplication)
                                        <option @if(old('building_features.building_heating_application_id', \App\Helpers\Hoomdossier::getMostCredibleValue($building->buildingFeatures(), 'building_heating_application_id')) == $buildingHeatingApplication->id) selected="selected"
                                                @endif value="{{ $buildingHeatingApplication->id }}">{{ $buildingHeatingApplication->name }}</option>
                                    @endforeach
                                </select>
                            @endcomponent
                        @endcomponent
        ...
        <div class="row" id="house-ventilation">
            <div class="col-xs-12 col-sm-12 col-md-4">
                ...
                @component('cooperation.tool.components.step-question', ['id' => 'house-ventilation-service', 'name' => 'services.'.$service->short.'.service_value_id', 'translation' => 'cooperation/tool/general-data/current-state.index.service.'.$service->short])
                    @component('cooperation.tool.components.input-group', ['inputType' => 'select', 'inputValues' => $service->values, 'userInputValues' => $building->buildingservices()->forMe()->where('service_id', $service->id)->get(), 'userInputColumn' => 'service_value_id'])
                        <select id="house-ventilation-service" class="form-control"
                                name="services[{{ $service->short }}][service_value_id]">
                            {{--5 is the "vraaggestuurd" value, we need this for a checkbox--}}
                            @foreach($service->values as $serviceValue)
                                <option data-calculate-value="{{$serviceValue->calculate_value}}"
                                        @if(old('services.' . $service->id.'.service_value_id', \App\Helpers\Hoomdossier::getMostCredibleValue($building->buildingservices()->where('service_id', $service->id), 'service_value_id')) == $serviceValue->id) selected="selected"
                                        @endif value="{{ $serviceValue->id }}">{{ $serviceValue->value }}</option>
                            @endforeach
                        </select>
                    @endcomponent
                    ...
                    <div class="form-group ">
                        <div class="input-group input-source-group">
                            <label class="checkbox-inline">
                                ...
                                <input value="true"
                                       @if(old('services.'.$service->id.'.extra.demand_driven', $isDemandDriven)) checked="checked"
                                       @endif type="checkbox"
                                       name="services[{{$service->short}}][extra][demand_driven]">
                            ...
                            <div class="input-group-btn">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    @foreach($answersForHouseVentilation as $houseVentilationAnswer)
                                        <?php $extraValues = $houseVentilationAnswer->extra; ?>

                                        @if((is_array($extraValues) && isset($extraValues['demand_driven'])) && $extraValues['demand_driven'])
                                            <li class="change-input-value"
                                                data-input-source-short="{{$houseVentilationAnswer->inputSource->short}}"
                                                data-input-value="true">
                                                <a href="">{{$houseVentilationAnswer->inputSource->name}}: Ja</a>
                                            </li>
                                        @endif
                                    @endforeach
                                </ul>
                    ...
                    <div class="form-group">
                        <div class="input-group input-source-group">
                            <label class="checkbox-inline">
                                ...
                                <input value="true"
                                       @if(old('services.'.$service->id.'.extra.heat_recovery', $hasHeatRecovery)) checked="checked"
                                       @endif type="checkbox"
                                       name="services[{{$service->short}}][extra][heat_recovery]">
                            ...
                            <div class="input-group-btn">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    @foreach($answersForHouseVentilation as $houseVentilationAnswer)
                                        ...
                                        @if((is_array($extraValues) && isset($extraValues['heat_recovery'])) && $extraValues['heat_recovery'])
                                            <li class="change-input-value"
                                                data-input-source-short="{{$houseVentilationAnswer->inputSource->short}}"
                                                data-input-value="true">
                                                <a href="">{{$houseVentilationAnswer->inputSource->name}}: Ja</a>
                ...
                @component('cooperation.tool.components.step-question', ['id' => 'services.'.$service->short.'.extra.value', 'translation' => 'cooperation/tool/general-data/current-state.index.service.'.$service->short])
                    @component('cooperation.tool.components.input-group', ['inputType' => 'input', 'userInputValues' => $building->buildingServices()->forMe()->where('service_id', $service->id)->get(), 'userInputColumn' => 'extra.value'])
                        ...
                        <input type="text" id="{{ $service->short }}" class="form-control"
                               name="services[{{ $service->short }}][extra][value]"
                               value="{{ old('services.' . $service->short.'.extra.value', \App\Helpers\Hoomdossier::getMostCredibleValue($building->buildingServices()->where('service_id', $service->id), 'extra.value')) }}">
                    ...
                    @component('cooperation.tool.components.step-question', ['id' => 'building-pv-panels-total-installed-power', 'name' => 'building_pv_panels.total_installed_power', 'translation' => 'cooperation/tool/general-data/current-state.index.installed-power', 'required' => false])
                        @component('cooperation.tool.components.input-group', ['inputType' => 'input', 'userInputValues' => $building->pvPanels()->forMe()->get(), 'userInputColumn' => 'total_installed_power'])
                            ...
                            <input type="text" class="form-control"
                                   name="building_pv_panels[total_installed_power]"
                                   value="{{ old('building_pv_panels.total_installed_power', \App\Helpers\Hoomdossier::getMostCredibleValue($building->pvPanels(), 'total_installed_power')) }}">
                        @endcomponent
                    @endcomponent
                </div>
                ...
                <div class="col-md-4">
                    @component('cooperation.tool.components.step-question', ['id' => 'services.'.$service->short.'.extra.year', 'translation' => 'cooperation/tool/general-data/current-state.index.service.'.$service->short.'.year', 'required' => false])
                        @component('cooperation.tool.components.input-group', ['inputType' => 'input', 'userInputValues' => $building->buildingServices()->forMe()->where('service_id', $service->id)->get(), 'userInputColumn' => 'extra.year'])
                            ...
                            <input type="text" class="form-control"
                                   name="services[{{ $service->short }}][extra][year]"
                                   value="{{ old('services.'.$service->short . '.extra.year', \App\Helpers\Hoomdossier::getMostCredibleValue($building->buildingServices()->where('service_id', $service->id), 'extra.year')) }}">
                        @endcomponent
                    @endcomponent
                </div>
