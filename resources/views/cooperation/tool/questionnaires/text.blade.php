    ...
    <div class="col-sm-12">
        <div class="form-group add-space{{ $errors->has('questions.'.$question->id) ? ' has-error' : '' }}">
            ...
            ['inputType' => 'input', 'userInputValues' => $question->questionAnswers()->forMe()->get(), 'userInputColumn' => 'answer'])
                <input @if($question->isRequired()) required="required" @endif name="questions[{{$question->id}}]" data-input-value="{{$question->name}}" placeholder="{{$question->name}}" value="{{old('questions.'.$question->id, $question->getAnswerForCurrentInputSource())}}" type="text" class="form-control">
            @endcomponent
