    ...
    <div class="col-sm-12">
        <div class="form-group add-space{{ $errors->has('questions.'.$question->id) ? ' has-error' : '' }}">
            ...
            ['inputType' => 'input', 'userInputValues' => $question->questionAnswers()->forMe()->get(),'userInputColumn' => 'answer'])
                ...
                <textarea @if($question->isRequired()) required="required" @endif name="questions[{{$question->id}}]" data-input-value="{{$question->id}}" class="form-control" rows="5">{{old('questions.'.$question->id, $question->getAnswerForCurrentInputSource())}}</textarea>
            @endcomponent
