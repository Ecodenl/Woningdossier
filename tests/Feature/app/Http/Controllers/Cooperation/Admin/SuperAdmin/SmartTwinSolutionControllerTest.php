...
final class SmartTwinSolutionControllerTest extends TestCase
{
    ...
    {
        $this->solution(self::CAVITY);
        $this->solution(self::SUN_BLINDS);
        $this->couple(self::CAVITY, MeasureApplication::findByShort('cavity-wall-insulation'));
    ...
    public function test_it_couples_a_product_to_a_measure(): void
    {
        ...
        $this->put($this->coupleRoute(), ['couplings' => [self::CAVITY => (string) $measure->id]])
    ...
    public function test_a_product_deliberately_left_uncoupled_is_written_down_as_such(): void
    {
        ...
        $this->put($this->coupleRoute(), [
            'couplings' => [self::SUN_BLINDS => SmartTwinSolutionController::NOT_COUPLED],
        ]);

        $mapping = $this->mappingFor(self::SUN_BLINDS);
    ...
    public function test_clearing_a_choice_returns_the_product_to_undecided(): void
    {
        ...
        $this->put($this->coupleRoute(), ['couplings' => [self::CAVITY => '']]);

        $this->assertNull($this->mappingFor(self::CAVITY));
    }
    ...
    public function test_recoupling_replaces_the_previous_measure_rather_than_adding_one(): void
    {
        ...
        $this->put($this->coupleRoute(), ['couplings' => [self::CAVITY => (string) $other->id]]);
    ...
    public function test_it_ignores_a_solution_that_is_not_in_the_catalogue(): void
    {
        ...
        $this->put($this->coupleRoute(), ['couplings' => ['Verzonnen|Id:Product' => (string) $measure->id]]);
    ...
    public function test_it_refuses_a_measure_that_does_not_exist(): void
    {
        ...
        $this->put($this->coupleRoute(), ['couplings' => [self::CAVITY => '999999']])
            ->assertSessionHasErrors('couplings.' . self::CAVITY);
    ...
    {
        // from_value is shared ground: the municipality mapping uses it too, and detaching without
        // a type would take those with it.
        $this->solution(self::CAVITY);
        DB::table('mappings')->insert(['from_value' => self::CAVITY, 'type' => 'something-else']);
    ...
    private function couple(string $externalId, MeasureApplication $measure): void
    {
        MappingService::init()
            ->from($externalId)
    ...
    private function mappingFor(string $externalId): ?Mapping
    {
        return Mapping::forType(SmartTwinSolution::mappingType())
            ->where('from_value', $externalId)
            ->first();
    }
